...
class Canvas:
    def __init__(self) -> None:
        ...
        if self.TOKEN == None:
            ...
            exit()
        self.auth_header = {'Authorization': 'Bearer ' + self.TOKEN} # setup the authorization header to be used later

        # ensure that COURSE_STATE is valid
        self.COURSE_STATE = os.environ.get('CANVAS_COURSE_STATE')
        if not self.COURSE_STATE in ["unpublished", "available", "completed", "deleted"]:
